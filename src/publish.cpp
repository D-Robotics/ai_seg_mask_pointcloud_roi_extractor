...
void AISegMaskPointCloudROIExtractor::pubImageCompressed(
    double timestamp,
    const std::string& frame_id,
    cv::Mat &image,
    rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr publisher,
...
void AISegMaskPointCloudROIExtractor::pubPointCloud(
...
template void AISegMaskPointCloudROIExtractor::pubPointCloud<pcl::PointXYZ>(
    const pcl::PointCloud<pcl::PointXYZ>&, const std::string&, double,
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr);

template void AISegMaskPointCloudROIExtractor::pubPointCloud<pcl::PointXYZRGB>(
    const pcl::PointCloud<pcl::PointXYZRGB>&, const std::string&, double,
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr);

template void AISegMaskPointCloudROIExtractor::pubPointCloud<pcl::PointXYZI>(
...
std::string AISegMaskPointCloudROIExtractor::inferImageEncoding(const cv::Mat& image)
{
    int type = image.type();
