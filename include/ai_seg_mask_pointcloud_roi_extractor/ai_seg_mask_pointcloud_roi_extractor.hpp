...
namespace robot::ai_seg_mask_pointcloud_roi_extractor
{
    ...
    class MaskPcRoiExtractor : public rclcpp::Node
    {
        ...
        private:
            ...
            /**
             * @brief Render and publish the ROI visual overlay image (bgr8).
             *
             * Colorizes the depth image (JET colormap), then overlays the exact
             * ROI class-ID mask pixels that are fed to semantic_map (same ROI
             * filtering as processROIPointClouds: same roi_conf confidence, same
             * class-ID mask, same depth range) so the overlay corresponds
             * 1:1 with the semantic information written to the map.
             *
             * @param depth_img Original depth image (read-only, CV_16UC1)
             * @param seg_mask_class_id Full-resolution class-ID mask (CV_8UC1, pixel = class_id + 1)
             * @param filtered_all_box_info Vector of filtered box infos (name, id, conf, rect)
             * @param timestamp Timestamp for publishing
             * @param frame_id Frame ID for publishing
             * @param roi_visual_pub Publisher for the rendered overlay image
             * @param img_width Camera image width (pixels)
             * @param img_height Camera image height (pixels)
             * @param min_depth Minimum depth (meters)
             * @param max_depth Maximum depth (meters)
             * @param roi_conf ROI class-name → confidence threshold map
             */
            void processROIVisual(const cv::Mat& depth_img,
                                  const cv::Mat& seg_mask_class_id,
                                  const std::vector<BoxInfo>& filtered_all_box_info,
                                  double timestamp,
                                  const std::string& frame_id,
...
RCLCPP_COMPONENTS_REGISTER_NODE(robot::ai_seg_mask_pointcloud_roi_extractor::MaskPcRoiExtractor)
